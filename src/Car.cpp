...
Car::Car( GLuint shaderProgramHandle, GLint mvpMtxUniformLocation, GLint normalMtxUniformLocation, GLint materialColorUniformLocation, GLint bound )
{
...
{
    float nextx = _carPosition.x + _moveSpeed * glm::cos( _carMoveAngle );
    float nextz = _carPosition.z + _moveSpeed * glm::sin( _carMoveAngle );
    if ( nextx < _carBound && nextz < _carBound && nextx > -_carBound && nextz > -_carBound )
    {
        _carPosition.x = nextx;
        _carPosition.z = nextz;
    }
...
void Car::turnLeft( ) { _carMoveAngle += 0.05; }

void Car::turnRight( ) { _carMoveAngle -= 0.05; }

void Car::setForwardDirection( ) { _forwardDirection = glm::vec3( glm::cos( _carMoveAngle ), 0.0f, glm::sin( _carMoveAngle ) ); }

glm::vec3 Car::getPosition( ) const { return _carPosition; }
